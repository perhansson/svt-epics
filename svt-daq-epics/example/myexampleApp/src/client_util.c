...
int open_socket(char* hostname, int portno) {
    ...
    // Create a socket  
    if(client_util_debug > 0) printf("[ open_socket ]: Opening new socket\n"); 
    ...
    if(client_util_debug > 1) printf("[ open_socket ]: New socket (%i) has been opened\n", socketfd); 
    ...
    // Connect to the host
    if(client_util_debug>1) printf("[ open_socket ]: Connecting to host %s ...\n", server->h_name);
